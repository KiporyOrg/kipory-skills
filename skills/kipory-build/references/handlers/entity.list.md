...
| `dataNullChecks` | object[] | no | — | Keep rows by whether a path inside `data` is empty or filled. A conversation list uses it to hide rows whose `deletedAt` is set. ⚠️ A row missing the key matches neither choice, so the record type has to always write it. And no index can serve a `data` path, so this reads every row. |
