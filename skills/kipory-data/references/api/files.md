...
| `recordType` | `string` | no | Narrow to files on records of ONE type, by the type's name — the same string a record's `recordType` carries. Implies `attached=record`. A type the project does not declare matches nothing rather than being refused: records can outlive their declaration. |
